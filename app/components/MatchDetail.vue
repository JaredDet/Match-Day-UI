    ...
    const players = lines.flatMap((count, line) =>
      Array.from({ length: count }, () => {
        ...
        return {
          ...
          captain: i === 2,
          x: side === 0 ? x : 100 - x,
          y: Math.max(8, Math.min(92, team.positions.find(position => position.number === numbers[i])?.y ?? 50)),
        };
      }),
    );
    return {
...
const detailTabs = [{ id: 'chronology', label: 'Cronología' }, { id: 'lineups', label: 'Alineaciones' }, { id: 'statistics', label: 'Estadísticas' }] as const
const activeTab = ref('lineups')
watch(() => props.match.id, () => { activeTab.value = 'lineups' })
...
const reportEvents = computed<ReportEvent[]>(() => {
  ...
  for (const squad of squads.value) {
    for (const player of [...squad.players, ...squad.substitutes]) {
      for (const event of eventsFor(squad.team, player.name, squad.side)) {
        ...
        items.push({ id: `${squad.side}-${player.number}-${event.kind}`, kind: event.kind === 'out' ? 'substitution' : event.kind, label: event.kind === 'out' ? 'Cambio' : event.label, minute: event.minute, added: event.added, side: squad.side, player: player.name, incoming: event.kind === 'out' ? substitution(squad.side).incoming : undefined, description: event.kind === 'out' && squad.side === 0 ? 'Cambio por lesión' : undefined })
      }
    }
    squad.team.goals.forEach((goal, index) => items.push({ id: `goal-${squad.side}-${index}`, kind: 'goal', label: goal.goal_type === 'own_goal' ? 'Autogol' : goal.goal_type === 'penalty' ? 'Gol de penal' : '¡Gol!', minute: goal.minute, added: goal.added_minute, side: squad.side, player: goal.player_name, assistPlayer: goal.goal_type === 'own_goal' ? undefined : goal.assist_player_name, description: goal.goal_type === 'own_goal' ? 'Gol a favor por autogol rival' : undefined }))
  }
  const extra: ReportEvent[] = [
    { id: 'shot-1', kind: 'shot', label: 'Tiro al arco', minute: 8, side: 0, player: 'M. Torres', goalkeeper: 'G. Ríos' },
    { id: 'corner-1', kind: 'corner', label: 'Tiro de esquina', minute: 9, side: 0, player: 'P. Vidal' },
    { id: 'foul-1', kind: 'foul', label: 'Falta', minute: 14, side: 1, player: 'T. Bravo' },
    { id: 'offside-1', kind: 'offside', label: 'Fuera de juego', minute: 27, side: 0, player: 'J. Rojas' },
    { id: 'shot-2', kind: 'shot', label: 'Tiro desviado', minute: 35, side: 1, player: 'L. Núñez' },
    { id: 'var-1', kind: 'var', label: 'Revisión del VAR', minute: 44, side: 0, description: 'Posible penal · Decisión: no se concede penal' },
    { id: 'corner-2', kind: 'corner', label: 'Tiro de esquina', minute: 54, side: 1, player: 'F. Muñoz' },
    { id: 'penalty-1', kind: 'penalty', label: 'Penal atajado', minute: 58, side: 1, player: 'D. Silva', goalkeeper: 'C. Herrera' },
    { id: 'foul-2', kind: 'foul', label: 'Falta', minute: 77, side: 0, player: 'A. Fuentes' },
  ]
  items.push(...extra.filter(event => event.minute <= minute.value))
  items.sort((a, b) => a.minute - b.minute || (a.added || 0) - (b.added || 0))
  const score = [0, 0]
  for (const event of items) if (event.kind === 'goal') { score[event.side] = score[event.side]! + 1; event.score = score.join(' – ') }
  if (props.match.shootout) {
    ...
    props.match.shootout.kicks.forEach(({side, player, outcome, sequence}) => {
      ...
      items.push({ id: `kick-${sequence}`, kind: 'shootout', label: outcome === 'scored' ? 'Penal convertido' : 'Penal fallado', minute: 120, side, player, sequence, outcome, score: penalties.join(' – ') })
    })
  }
  return items
})
...
<template>
  <section class="field-section" aria-labelledby="field-title">
    ...
    <div class="field-legend">
      <span v-for="squad in squads" :key="squad.team.id"
        ...
        <strong>{{ squad.formation }}{{ squad.team.formation_variant === "closed" ? " · Cerrado" : squad.team.formation_variant === "open" ? " · Abierto" : "" }}</strong></span
      >
    </div>
    <div
      class="pitch-scroll"
  ...
  <div class="detail-tabs" role="tablist" aria-label="Información del partido">
    <button v-for="(tab, index) in detailTabs" :id="`detail-tab-${tab.id}`" :key="tab.id" role="tab" :aria-selected="activeTab === tab.id" :aria-controls="`detail-panel-${tab.id}`" :tabindex="activeTab === tab.id ? 0 : -1" @click="activeTab = tab.id" @keydown="navigateTab($event, index)">{{ tab.label }}</button>
  </div>
  <div v-show="activeTab === 'lineups'" id="detail-panel-lineups" role="tabpanel" aria-labelledby="detail-tab-lineups" tabindex="0">
    ...
    <div class="squad-grid">
      <section v-for="squad in squads" :key="squad.team.id" class="squad-panel">
        <header>
          ...
          <span>{{ squad.formation }}{{ squad.team.formation_variant === "closed" ? " · Cerrado" : squad.team.formation_variant === "open" ? " · Abierto" : "" }}</span>
        </header>
  ...
  <div v-show="activeTab === 'chronology'" id="detail-panel-chronology" role="tabpanel" aria-labelledby="detail-tab-chronology" tabindex="0"><MatchReport view="chronology" :match="match" :events="reportEvents" /></div>
  <div v-show="activeTab === 'statistics'" id="detail-panel-statistics" role="tabpanel" aria-labelledby="detail-tab-statistics" tabindex="0"><MatchReport view="statistics" :match="match" :events="reportEvents" /></div>
</template>
...
.football-field{background-color:#51863b;background-image:radial-gradient(ellipse at 45% 35%,#a5ce7026,transparent 75%)}

.detail-tabs{display:grid;grid-template-columns:repeat(3,1fr);border-bottom:1px solid #555;margin-top:25px;background:#1c1c1c}.detail-tabs button{padding:17px 8px;color:#a5a5a5;text-transform:uppercase;font-size:10px;font-weight:600;border-bottom:2px solid transparent}.detail-tabs button[aria-selected="true"]{background:#2b2e33;color:#fff;border-bottom-color:#e1e1e1}.detail-tabs button:hover{color:white}.detail-tabs button:focus-visible{outline:2px solid #aaa;outline-offset:-4px}[role="tabpanel"]:focus-visible{outline:1px solid #777;outline-offset:3px}
</style>
