...
export function shootoutRows(shootout: Shootout) {
  const sides = [0, 1].map((side) =>
    shootout.kicks.filter((kick) => kick.side === side).sort((a, b) => a.sequence - b.sequence),
  );
  const nextCount = shootout.nextSide === null ? 0 : sides[shootout.nextSide]!.length + 1;
  const rounds = Math.max(
    5,
    ...sides.map((side) => side.length),
    shootout.status === "in_progress" ? nextCount : 0,
  );
  return sides.map((kicks, side) => ({
    ...
    cells: Array.from({ length: rounds }, (_, index) => ({
      ...
      next:
        shootout.status === "in_progress" && shootout.nextSide === side && index === kicks.length,
    })),
  }));
}
export function demoShootout(finished: boolean): Shootout {
  ...
  return {
    ...
    nextSide: finished ? null : 0,
    kicks: shots.map(([side, player, outcome], index) => ({
      side,
      player,
      outcome,
      sequence: index + 1,
