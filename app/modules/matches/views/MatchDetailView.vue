  ...
  <main class="match-page">
    ...
    <div class="match-page-heading"><h1>{{ match.home_team.name }} <span>vs.</span> {{ match.away_team.name }}</h1><p>Vista de demostración · Datos ficticios</p></div>
        ...
        <article class="match-card detail-summary" :class="{ 'is-live': match.status === 'live' }">
            ...
            <div class="fixture"><div class="team"><span class="crest" :style="{ '--team-color': teamColor(match.home_team.name) }">{{ initials(match.home_team.name) }}</span><h4><NuxtLink :to="`/teams/${match.home_team.id}`">{{ match.home_team.name }}</NuxtLink></h4></div><div class="score"><template v-if="match.status === 'scheduled'"><strong class="kickoff">{{ time(match.scheduled_at) }}</strong></template><template v-else><strong>{{ match.home_team.score }} <em>–</em> {{ match.away_team.score }}</strong></template></div><div class="team"><span class="crest" :style="{ '--team-color': teamColor(match.away_team.name) }">{{ initials(match.away_team.name) }}</span><h4><NuxtLink :to="`/teams/${match.away_team.id}`">{{ match.away_team.name }}</NuxtLink></h4></div></div>
        ...
        <NuxtLink :to="`/matches/manage?match=${match.id}`" class="back-to-matches">Operar partido</NuxtLink><OperatedMatchReport v-if="operations[match.id]" :match-id="match.id" /><MatchDetail v-else :match="match" />
  </main>
</template>
...
.match-page{max-width:1080px;padding-top:26px;padding-bottom:40px}.back-to-matches{display:inline-flex;align-items:center;gap:9px;color:var(--ui-muted, #aaa);font-size:12px;padding:8px 0}.back-to-matches:hover{color:var(--ui-text, #fff)}.match-page-heading{margin:22px 0;text-align:center}.match-page-heading h1{font-size:24px;letter-spacing:-.5px;line-height:1.5}.match-page-heading h1 span{color:var(--ui-muted, #888);font-size:17px;font-weight:400;margin:0 7px}.match-page-heading p{font-size:10px;color:var(--ui-muted, #929292);margin-top:5px}.detail-date{font-size:12px;color:var(--ui-muted, #999);text-align:center}.detail-summary{max-width:620px}.detail-summary:hover{border-color:var(--ui-border, #303030)}
