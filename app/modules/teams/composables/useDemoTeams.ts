...
export function useDemoTeams() {
  ...
  function create(draft: TeamRegistrationDraft) {
    if (
      teams.value.some(
        (team) =>
          team.name.trim().toLocaleLowerCase("es") === draft.name.trim().toLocaleLowerCase("es"),
      )
    )
    ...
    teams.value.push({
      ...
      coach: draft.head_coach_name,
      city: draft.city,
      stadium: draft.stadium_name,
      founded: draft.founded_year ?? new Date().getFullYear(),
    });
    rosters.value[id] = draft.players.map((player, index) => ({
      ...player,
      id: `${id}-${index}`,
      preferred_position: (player.preferred_position || null) as TeamPlayer["preferred_position"],
      is_captain: false,
    }));
    return id;
  }
  function update(id: string, name: string, coach: string) {
    ...
    if (!name.trim() || !coach.trim())
      throw new Error("Nombre y director técnico son obligatorios.");
    if (
      teams.value.some(
        (item) =>
          item.id !== id &&
          item.name.trim().toLocaleLowerCase("es") === name.trim().toLocaleLowerCase("es"),
      )
    )
      throw new Error("Ya existe un equipo con ese nombre.");
    team.name = name.trim();
    team.coach = coach.trim();
  }
  function replaceRoster(teamId: string, players: TeamPlayer[]) {
    rosters.value[teamId] = players.map((player) => ({ ...player }));
  }
  function savePlayer(
    teamId: string,
    draft: Omit<TeamPlayer, "id" | "is_captain">,
    playerId?: string,
  ) {
    ...
    if (
      roster.some(
        (player) =>
          player.id !== playerId &&
          player.name.trim().toLocaleLowerCase("es") === draft.name.trim().toLocaleLowerCase("es"),
      )
    )
      throw new Error("Ese jugador ya está en la plantilla.");
    if (
      draft.preferred_shirt_number != null &&
      roster.some(
        (player) =>
          player.id !== playerId && player.preferred_shirt_number === draft.preferred_shirt_number,
      )
    )
      throw new Error("Ese dorsal ya está ocupado.");
  ...
  function setCaptain(teamId: string, playerId: string) {
    ...
    if (!roster.some((player) => player.id === playerId))
      throw new Error("Selecciona un jugador de la plantilla.");
    roster.forEach((player) => {
      player.is_captain = player.id === playerId;
    });
  }
  return {
    teams,
    ...
    savePlayer,
    setCaptain,
    teamById: (id: string) => teams.value.find((team) => team.id === id),
  };
}
