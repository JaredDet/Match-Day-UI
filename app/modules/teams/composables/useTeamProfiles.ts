...
export function useTeamProfiles() { const { teams, rosters } = useDemoTeams()
  const { matches } = useDemoMatches(); return computed(() => createTeamProfiles(matches.value, teams.value, rosters.value)) }
