import type {
  ...
  NavigationSignal,
  Recommendation,
} from "~/modules/recommendations/types/recommendations";
...
export function cleanHistory(value: unknown, now: number): NavigationSignal[] {
  ...
  return value
    .filter(
      (s): s is NavigationSignal =>
        !!s &&
        typeof s === "object" &&
        typeof s.key === "string" &&
        /^(team|player|news|match|tournament):[\w-]+$/.test(s.key) &&
        Number.isFinite(s.lastAt) &&
        s.lastAt <= now &&
        s.lastAt >= now - HISTORY_DAYS * DAY &&
        typeof s.day === "string" &&
        s.day === new Date(s.lastAt).toISOString().slice(0, 10) &&
...
export function rankContent(
  ...
  const teamInterest = new Map<string, number>(),
    tournamentInterest = new Map<string, number>();
  ...
  for (const signal of cleanHistory(history, now)) {
    ...
    candidate.teams.forEach((id) =>
      teamInterest.set(
        id,
        (teamInterest.get(id) ?? 0) + weight / Math.max(1, candidate.teams.length),
      ),
    );
    candidate.tournaments.forEach((id) =>
      tournamentInterest.set(id, (tournamentInterest.get(id) ?? 0) + weight),
    );
  }
  ...
  return candidates
    .map((candidate) => {
      ...
      const tournamentScore = Math.max(
        0,
        ...candidate.tournaments.map((id) => tournamentInterest.get(id) ?? 0),
      );
      ...
      const preferredTeam =
        strongestTeamInterest > 0 &&
        candidate.teams.some((id) => (teamInterest.get(id) ?? 0) === strongestTeamInterest);
      const reason =
        affinity > 0
          ? preferredTeam
            ? "De uno de los equipos que más has consultado"
            : teamScore >= tournamentScore * 0.7
              ? "Relacionado con equipos que has consultado"
              : "Relacionado con torneos que has visitado"
          : candidate.live
            ? "Un partido que se está jugando"
      ...
      return {
        ...
        preferredTeam,
        score:
          affinity + freshness + (candidate.live ? 1 : 0) - (seen.has(candidate.key) ? 0.5 : 0),
        reason,
      };
    })
...
export function recommendationSections(ranked: Recommendation[], history: NavigationSignal[]) {
  ...
  const matches = [
    ...
    ...matchCandidates.filter(
      (item) => item.preferredTeam && !directMatches.some((selected) => selected.key === item.key),
    ),
  ].slice(0, 4);
  ...
  const discoveryCandidates = ranked
    .filter(
      (item) =>
        !["player", "team"].includes(item.kind) && !displayed.has(item.key) && !seen.has(item.key),
    )
