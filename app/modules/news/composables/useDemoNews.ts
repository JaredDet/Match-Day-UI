...
export function useDemoNews() {
  const items = useState<DemoNews[]>("demo-news", () =>
    news.map((item) => ({
      ...item,
      content: { children: [...item.content.children] },
    })),
  );
  const publishedNews = computed<NewsPreview[]>(() =>
    items.value
      .filter((item) => item.status === "PUBLISHED")
      .sort((a, b) => (b.published_at ?? "").localeCompare(a.published_at ?? ""))
      .map(({ content, ...item }) => ({
        ...item,
        preview: newsPreview(content.children),
      })),
  );
  function save(
    ...
    if (id && (!current || current.status !== "DRAFT"))
    ...
    for (const paragraph of input.content.children) {
      ...
      for (const token of content.match(/<[^>]*>/g) ?? []) {
        ...
        if (token.startsWith("</")) {
          if (tags.pop() !== token[2])
            throw new Error("Revisa el cierre de las etiquetas de formato.");
  ...
  function act(
    ...
    if (!item || item.status === "PUBLISHED")
      throw new Error("La noticia publicada es de solo lectura.");
    if (action === "unschedule") {
      if (item.status !== "SCHEDULED") throw new Error("La noticia no está programada.");
      item.status = "DRAFT";
      item.scheduled_at = null;
      return;
    }
    if (action === "delete") {
      if (item.status !== "DRAFT") throw new Error("Solo se pueden eliminar borradores.");
      items.value = items.value.filter((n) => n.id !== id);
      return;
    }
    if (!item.content.children.some((p) => plainNewsText(p)))
      throw new Error("Añade contenido antes de publicar o programar.");
    if (action === "schedule") {
      if (
        item.status !== "DRAFT" ||
        !date ||
        ...
        throw new Error("Selecciona una fecha futura para el borrador.");
      item.status = "SCHEDULED";
      item.scheduled_at = new Date(date).toISOString();
    } else {
  ...
  function publishDue() {
    const due = items.value.filter(
      (item) => item.status === "SCHEDULED" && Date.parse(item.scheduled_at!) <= Date.now(),
    );
    due.forEach((item) => act(item.id, "publish"));
    return due.length;
  }
