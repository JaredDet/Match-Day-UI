...
const source = await readFile(
  new URL("../app/modules/recommendations/utils/ranking.ts", import.meta.url),
  "utf8",
);
const { cleanHistory, recordNavigation, rankContent, recommendationSections } = await import(
  `data:text/javascript;base64,${Buffer.from(stripTypeScriptTypes(source)).toString("base64")}`
);
const now = Date.parse("2026-09-21T12:00:00Z");
const candidate = (key, kind, teams = [], tournaments = []) => ({
  key,
  kind,
  teams,
  tournaments,
...
const catalog = [
  candidate("team:a", "team", ["a"]),
  candidate("team:b", "team", ["b"]),
  candidate("news:a", "news", ["a"]),
  candidate("news:b", "news", ["b"]),
  candidate("match:a", "match", ["a", "c"]),
  candidate("tournament:cup", "tournament", [], ["cup"]),
];
...
test("navigation produces related news and matches without explicit interests", () => {
  ...
  assert.ok(
    ranked.find((c) => c.key === "news:a").score > ranked.find((c) => c.key === "news:b").score,
  );
...
test("interests decay and visits across days accumulate", () => {
  ...
  assert.ok(
    ranked.find((c) => c.key === "news:b").affinity >
      ranked.find((c) => c.key === "news:a").affinity,
  );
  ...
  assert.equal(
    cleanHistory(
      [
        valid,
        { ...valid, lastAt: "bad" },
        { ...valid, lastAt: Infinity },
        { ...valid, visits: 999 },
        { ...valid, lastAt: now + 1000 },
        { ...valid, lastAt: now - 31 * 86400000 },
      ],
      now,
    ).length,
    1,
...
test("cold start, clearing and discovery do not fabricate personalized reasons or repeat cards", () => {
  ...
  assert.ok(
    recommendationSections(rankContent(catalog, visited, now), visited).discovery.every(
      (c) => c.key !== "team:a",
    ),
  );
});
test("tournament interest reaches content associated with that tournament; unknown demo IDs have no effect", () => {
  ...
  assert.ok(
    rankContent(content, recordNavigation([], "team:deleted", 30, now), now).every(
      (c) => c.affinity === 0,
    ),
  );
});
